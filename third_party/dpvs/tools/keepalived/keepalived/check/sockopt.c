...
static inline int sockopt_msg_send(int clt_fd,
        const struct keepalived_dpvs_sock_msg *hdr,
        const char *data, int data_len)
{
    ...
    len = sizeof(struct keepalived_dpvs_sock_msg);
    // len = 24;
...
static inline int sockopt_msg_recv(int clt_fd, struct keepalived_dpvs_sock_msg_reply *reply_hdr, 
        void **out, size_t *out_len)
{
    ...
    len = sizeof(struct keepalived_dpvs_sock_msg_reply);
    // len = 88;
...
{
    struct keepalived_dpvs_sock_msg *msg;
    struct keepalived_dpvs_sock_msg_reply reply_hdr;
    ...
    memcpy(dpvs_ipc_file, "/var/run/dpvs.ipc", sizeof("/var/run/dpvs.ipc"));

    // green color
    printf("\033[32m[INFO] dpvs_setsockopt   dpvs_ipc_file: %s\033[0m\n", dpvs_ipc_file);
    ...
    msg_len = sizeof(struct keepalived_dpvs_sock_msg);
    msg = malloc(msg_len);
    if (NULL == msg) {
        fprintf(stderr, "[%s] no memory\n", __func__);
        return -ESOCKOPT_INVAL;
    ...
    // green color
    printf("\033[32m[INFO] After connect\033[0m\n");

    memset(msg, 0, msg_len);
    msg->version = SOCKOPT_VERSION;
    msg->id = cmd;
    msg->type = KEEPALIVED_SOCKOPT_SET;
    msg->len = in_len;
    res = sockopt_msg_send(clt_fd, msg, in, in_len);

    // green color
    printf("\033[32m[INFO] After sockopt_msg_send\033[0m\n");
    ...
    // green color
    printf("\033[32m[INFO] After sockopt_msg_recv\033[0m\n");
        ...
        void **out, size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    msg->version = SOCKOPT_VERSION;
    msg->id = cmd;
    msg->type = KEEPALIVED_SOCKOPT_GET;
    msg->len = in_len;
    res = sockopt_msg_send(clt_fd, msg, in, in_len);
