...
enum keepalived_sockopt_type {
    KEEPALIVED_SOCKOPT_GET = 0,
    KEEPALIVED_SOCKOPT_SET,
    KEEPALIVED_SOCKOPT_TYPE_MAX,
};

struct keepalived_dpvs_sock_msg {
    uint32_t version;
    sockoptid_t id;
    enum keepalived_sockopt_type type;
    size_t len;
    char data[0];
};

struct keepalived_dpvs_sock_msg_reply {
    uint32_t version;
    sockoptid_t id;
    enum keepalived_sockopt_type type;
    int errcode;
