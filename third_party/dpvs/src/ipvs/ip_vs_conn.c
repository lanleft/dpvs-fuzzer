                                  ...
                                  struct dp_vs_dest *dest, uint32_t flags)
{
    ...
    printf("\033[32m[INFO] dp_vs_conn_new\033[0m\n");
    ...
    if (dest->fwdmode == DPVS_FWD_MODE_FNAT) {
        err = dp_vs_laddr_bind(new, dest->svc);
        printf("\033[32m[INFO] dp_vs_laddr_bind, err: %d\033[0m\n", err);
        if (err != EDPVS_OK)
            goto unbind_dest;
    }
...
{
    // printf function name and refcnt
    printf("[INFO] %s conn: %p, cnt: %d\n", __func__, conn, rte_atomic32_read(&conn->refcnt));
    if (conn->flags & DPVS_CONN_F_ONE_PACKET) {
...
static int conn_init_lcore(void *arg)
{
    ...
    // green color
    printf("\033[32m[INFO] conn_init_lcore: %d\033[0m\n", cid);
