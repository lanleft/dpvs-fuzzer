...
int ip4_defrag(struct rte_mbuf *mbuf, int user)
{
    ...
    printf("\033[32m[INFO] ip4_defrag\033[0m\n");
    ...
    default: /* error happened */
        printf("\033[31m[ERR] ip4_defrag rte_pktmbuf_free: %d\033[0m\n", err);
...
static int ipv4_local_in_fin(struct rte_mbuf *mbuf)
{
    ...
    printf("\033[32m[INFO] ipv4_local_in_fin\033[0m\n");
...
static int ipv4_local_in(struct rte_mbuf *mbuf)
{
    ...
    // print green color
    printf("\033[32m[INFO] ipv4_local_in\033[0m\n");
...
int ipv4_rcv_fin(struct rte_mbuf *mbuf)
{
    ...
    // green color
    printf("\033[32m[INFO] ipv4_rcv_fin\033[0m\n");
...
static int ipv4_rcv(struct rte_mbuf *mbuf, struct netif_port *port)
{
    ...
    // print green color
    printf("\033[32m[INFO] ipv4_rcv\033[0m\n");
    ...
    if (unlikely(!(port->flag & NETIF_PORT_FLAG_RX_IP_CSUM_OFFLOAD))) {
        uint16_t csum = rte_raw_cksum(iph, hlen);
        if (unlikely(csum != 0xFFFF)){
            printf("[ERR] csum error, correct checksum: %x\n", csum);
            goto csum_error;
        }
        printf("[INFO] correct csum: %x\n", csum); 
    }
...
int ipv4_init(void)
{
    ...
    printf("[INFO] Calling netif_register_pkt for ipv4 packet type\n");
