...
static int set_log_level(char *log_level)
{
    ...
    if (!log_level) {
        ...
        printf("[INFO] set log level to DEBUG\n");
        rte_log_set_global_level(RTE_LOG_DEBUG);
        set_log_level_dynamic_types("user[0-9]", RTE_LOG_DEBUG);
    } else {
...
static void log_file_handler(vector_t tokens)
{
    ...
    RTE_LOG(INFO, CFG_FILE, "after set_log_file\n");
    FREE_PTR(log_file);
}
...
{
    printf("[INFO] install_global_keywords\n");
