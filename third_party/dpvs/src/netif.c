...
// int netif_pktpool_nb_mbuf = 1024;
...
// int netif_pktpool_mbuf_cache = 0;
...
{
    printf("[INFO] Starting install_netif_keywords\n");
...
static inline void netif_pktmbuf_pool_init(void)
{
    ...
    for (i = 0; i < get_numa_nodes(); i++) {
        ...
        if (!pktmbuf_pool[i])
        {
            printf("[ERROR] Cannot init mbuf pool on socket %d\n", i);
...
int netif_rcv_mbuf(struct netif_port *dev, lcoreid_t cid, struct rte_mbuf *mbuf, bool pkts_from_ring)
{
    ...
    if (eth_hdr->ether_type == htons(ETH_P_8021Q) ||
        ...
        if (vlan_rcv(mbuf, netif_port_get(mbuf->port)) != EDPVS_OK) // checking port when receiving packet
            goto drop;
    ...
    printf("[INFO] eth_hdr->ether_type: 0x%x\n", eth_hdr->ether_type);
    pt = pkt_type_get(eth_hdr->ether_type, dev); // why this place doesn't create mbuf_copied?
    printf("[INFO] pt: %p\n", pt);
    if (NULL == pt) {
        if (!forward2kni) {
            kni_ingress(mbuf, dev); // requires intertactions with KNI (kernel network interface)
            goto done;
        }
        goto drop;
    }
...
static void dpdk_port_setup(struct netif_port *dev)
{
    ...
    // create dev_info of dev
    dev->dev_info.max_rx_queues = 1;
    dev->dev_info.max_tx_queues = 1;
    dev->dev_info.max_rx_pktlen = 1024;
    dev->dev_info.max_mac_addrs = 1;
    dev->dev_info.max_hash_mac_addrs = 1;
    dev->dev_info.max_vfs = 0;

    // dev_info.driver_name = "net-virtio";
    dev->dev_info.driver_name = (char *)malloc(20);
    memset(dev->dev_info.driver_name, 0, 20);
    strcpy(dev->dev_info.driver_name, "net-virtio");
...
static void netif_port_init(void)
{
    ...
    // printf("Starting netif_port_init\n");
    // nports = 1;

    // // add a new port to port_list
    // struct port_conf_stream *port_cfg = malloc(sizeof(struct port_conf_stream));
    // port_cfg->port_id = -1;
    // strcpy(port_cfg->kni_name, "dpdk0\0");
    // list_add_tail(&port_cfg->port_list_node, &port_list);

    nports = dpvs_rte_eth_dev_count();
    printf("[INFO] nports: %d\n", nports);
    if (nports <= 0)
    ...
    printf("Initializing port_tab_init\n");
    port_tab_init();
    printf("Initializing port_ntab_init\n");
    port_ntab_init();
    printf("Initializing kni_init\n");
    kni_init();

    printf("Initializing port_name_alloc\n");
    for (pid = 0; pid < nports; pid++) {
        ...
        printf("Initializing netif_port_register\n");
...
int netif_vdevs_add(void)
{
    ...
    port_id_end = max1(port_id_end, phy_pid_end);
    ...
    if (!list_empty(&bond_list)) {
        ...
        port_id_end = max1(port_id_end, bond_pid_end);
...
int netif_init(void)
{
    printf("[INFO] Calling netif_pktmbuf_pool_init\n");
    netif_pktmbuf_pool_init();
    printf("[INFO] Calling netif_arp_ring_init\n");
    netif_arp_ring_init();
    printf("[INFO] Calling netif_pkt_type_tab_init\n");
    netif_pkt_type_tab_init();
    printf("[INFO] Calling netif_port_init\n");
    netif_port_init();
    printf("[INFO] Calling netif_lcore_init\n");
    netif_lcore_init();
